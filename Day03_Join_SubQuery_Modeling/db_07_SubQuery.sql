...
SELECT deptno, ename, job, sal FROM emp WHERE sal > (SELECT AVG(sal) FROM emp WHERE deptno = 20);
...
SELECT ename, sal, job, hiredate, deptno FROM emp WHERE sal > ALL (SELECT sal + ifnull(comm, 0) FROM emp WHERE job = 'SALESMAN');
...
FROM emp
WHERE (deptno, hiredate) IN (SELECT ifnull(deptno, 0), MIN(hiredate) FROM emp GROUP BY deptno);
